...
const  getInter = async function (req,res){
    try{
        ...
        let savedCollege = await collegeModel.find({name:name}).select({isDeleted:0,createdAt:0,updatedAt:0,__v:0,_id:0})
        
        let collegeId= savedCollege._Id
        if(collegeId == collegeId){
        let savedIntern = await interModel.find(collegeId).select({isDeleted:0,createdAt:0,updatedAt:0,__v:0,collegeId:0})
        const  interns =[... savedIntern];
        savedCollege.push(interns)
